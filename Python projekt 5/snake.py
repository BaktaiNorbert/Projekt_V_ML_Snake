...
async def RunGame():
    ...
    while True:
        ...
        player.rotation = inp
        if not pause:
            if player.position.x == screen.resolution.x + 1 or player.position.x == 1 or player.position.y == screen.resolution.y or player.position.y == 0: GameOver() #Határt ér a snake
            for position in last_positions:
                if player.position.x == position.x and player.position.y == position.y and last_positions.index(position) != len(last_positions)-1: GameOver() #Belemnegy a farkába a snake
